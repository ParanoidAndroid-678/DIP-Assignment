...
def add_landmarks_to_image(image_path, landmarks, size=5):
    image = io.imread(image_path)
    for points in landmarks:
        for (x, y) in points:
            image = cv2.circle(image, (int(x), int(y)), size, (0, 255, 0), -1)
    return image

def on_upload_image(image, model, size):
    landmarks = detect_landmarks(image.name)
    image_with_landmarks = add_landmarks_to_image(image.name, landmarks, size=SIZE_TO_CLICK_SIZE[size])
    return image_with_landmarks, image_with_landmarks
...
def main():
    ...
    with gr.Blocks() as demo:
        gr.Markdown(
            ...
            This demo allows you to automatically detect and edit facial features 
            using DragGAN.
            """
        )
        ...
        with gr.Row():
            ...
            with gr.Column():
                upload = gr.UploadButton("Upload Image")
                uploaded_image = gr.Image(label='Uploaded Image').style(height=512, width=512)
                with gr.Tabs():
        ...
        upload.upload(on_upload_image, inputs=[uploaded_image, model, size], outputs=[image, mask])
