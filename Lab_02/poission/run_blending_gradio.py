...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    loss = F.mse_loss(foreground_laplacian_masked, blended_laplacian_masked)
    
    return loss

# Perform Poission image blending
def blending(foreground_image_original, background_image_original, dx, dy, polygon_state):
    ...
    bg_mask_tensor = torch.from_numpy(background_mask).to(device).unsqueeze(0).unsqueeze(0).float() / 255.0
    ...
    for step in range(iter_num):
        ...
        if step % 50 == 0:
            print(f'Optimizer step: {step}, Laplacian distance loss: {loss.item()}')
        if step == (iter_num // 2):
...
def close_polygon_and_reset_dx(img_original, polygon_state, dx, dy,  background_image_original):
    ...
    Returns:
        tuple: Updated imge with polygon, updated polygon state,  updated background image, and reset dx value.
    """
    # Close polygon
    ...
    # Update background imge
    update_background = update_background(background_image_original, updated_polygon_state, 0, dy)
    return img_with_poly, updated_polygon_state, update_background, new_dx
